...
size_t nb_cols,nb_rows,length; // row and column
    ...
    void Mult(FVVect<T_> &) ;
    void Mult(const FVPoint2D<double> &,  FVPoint2D<double> &) ;
    void Mult(FVPoint2D<double>  &) ;    
    void Mult(const FVPoint3D<double> &,  FVPoint3D<double> &) ;
    void Mult(FVPoint3D<double>  &) ;      
    void Mult(const FVPoint4D<double> &,  FVPoint4D<double> &) ;
    void Mult(FVPoint4D<double>  &) ;   
    void leftMatrixMult(FVVect<T_> &) ;    
    void rightMatrixMult(FVVect<T_> &) ;       
    ...
    void DagMatrix(FVDenseM<double> &)   ;  // compute the pseudo inverse
...
void FVDenseM<T_>::setColumn(size_t j, const FVVect<T_> &column) 
    ...
    if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<"index out of range, error code="<<err<<endl;
#endif
    for(size_t i=0;i<nb_rows;i++)  a[nb_cols*i+j]=column[i];
    }        
...
template<class T_>
void FVDenseM<T_>::Mult(FVVect<T_> &y) 
    {
register size_t pos_i; 
register size_t i,j; 
FVVect<double> x(y);
...
template<class T_>    
void FVDenseM<T_>::Mult(const FVPoint2D<double> &X,  FVPoint2D<double> &Y) 
{
#ifdef _DEBUGS
size_t err=(nb_cols!=2)+2*(nb_rows!=2);
if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<
               "index out of range, error code="<<err<<endl;
#endif 
Y.x=a[0]*X.x+a[1]*X.y;
Y.y=a[2]*X.x+a[3]*X.y;
}
template<class T_>
void FVDenseM<T_>::Mult(FVPoint2D<double>  &X)    
{
FVPoint2D<double> Y;
FVDenseM<T_>::Mult(X,Y);
X=Y;
}    
template<class T_>
void FVDenseM<T_>::Mult(const FVPoint3D<double> &X,  FVPoint3D<double> &Y)
{
#ifdef _DEBUGS
size_t err=(nb_cols!=3)+2*(nb_rows!=3);
if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<
               "index out of range, error code="<<err<<endl;
#endif 
...
template<class T_>
void FVDenseM<T_>::Mult(FVPoint3D<double>  &X)      
{
FVPoint3D<double> Y;
FVDenseM<T_>::Mult(X,Y);
X=Y;
...
template<class T_>
void FVDenseM<T_>::Mult(const FVPoint4D<double> &X,  FVPoint4D<double> &Y)
{
#ifdef _DEBUGS
size_t err=(nb_cols!=4)+2*(nb_rows!=4);
if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<
               "index out of range, error code="<<err<<endl;
#endif     
Y.x=a[0]*X.x+a[1]*X.y+a[2]*X.z+a[3]*X.t;
Y.y=a[4]*X.x+a[5]*X.y+a[6]*X.z+a[7]*X.t;
Y.z=a[8]*X.x+a[9]*X.y+a[10]*X.z+a[11]*X.t;
Y.t=a[12]*X.x+a[13]*X.y+a[14]*X.z+a[15]*X.t;
}
template<class T_>
void FVDenseM<T_>::Mult(FVPoint4D<double>  &X)  
{
FVPoint4D<double> Y;
FVDenseM<T_>::Mult(X,Y);
X=Y;
...
// compute the Matricial production D*A* where D is a diagonal matrix    
template<class T_>
void FVDenseM<T_>::leftMatrixMult(FVVect<T_> &D) 
    {
register size_t pos_i; 
register size_t i,j; 
#ifdef _DEBUGS
    size_t err=(D.size()!=nb_rows);
    if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<"index out of range, error code="<<err<<endl;
#endif  
    pos_i=0;
    for (i=0; i<nb_rows; i++)
...
// compute the Matricial production A*D where D is a diagonal matrix    
template<class T_>
void FVDenseM<T_>::rightMatrixMult(FVVect<T_> &D) 
    {
register size_t pos_i; 
register size_t i,j; 
#ifdef _DEBUGS
    size_t err=*(D.size()!=nb_cols);
    if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<"index out of range, error code="<<err<<endl;
#endif   
    pos_i=0;
    for (i=0; i<nb_rows; i++)
...
//
template<class T_>
void FVDenseM<T_>::DagMatrix(FVDenseM<double> &q)   
{
    // Caution: q is the Transpose Matrix of the decomposition
 #ifdef _DEBUGS
    size_t err=(q.nb_rows!=nb_cols)+2*(q.nb_cols!=nb_rows); // must have the same dim than the tranpose matrix
    if(err) cout<<" in file "<<__FILE__<<", line "<<__LINE__<<"index out of range, error code="<<err<<endl;
#endif      
for(size_t i=0;i<nb_rows;i++)
    for(size_t j=0;j<nb_cols;j++)
        q.a[nb_rows*j+i]=a[nb_cols*i+j]; // set the transpose matrix      
FVDenseM<double> Q(nb_rows);
FVDenseM<T_>::QRFactorize(Q); // R is in a  
int m=(int) nb_rows;
int n=(int) nb_cols;
// forward substitution
for(int i=0;i<n; i++)
    ...
    for(int k=0;k<m;k++)
        {
        for(int j=0;j<i;j++)
              q.a[k+m*i]-=q.a[k+m*j]*a[j*n+i];
        q.a[k+m*i]/=a[i*n+i];
        }
    }     
//bacbward substitution
for(int i=n-1;i>=0; i--)
    {
    for(int k=0;k<m;k++)
        {
        for(int  j=i+1;j<n;j++)
              q.a[k+m*i]-=q.a[k+m*j]*a[i*n+j];
        q.a[k+m*i]/=a[i*n+i];
        }
    }
