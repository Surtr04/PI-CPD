...
void FVio::put(FVVect <double> &u, const double time, const string &name)
{
   ...
   // the FVPoint1D version 
void FVio::put(FVVect <FVPoint1D<double> >&u, const double time,const  string &name)
{
    if(_access!=FVWRITE)
    {
        cout<<" wrong access code: should be FVWRITE"<<endl; return;
    ...
    _of<<"name=\""<<name<<"\">"<<endl;
    for(size_t i=0;i<u.size(); i++)
         _of <<scientific << setprecision(FVPRECISION) << setw(FVCHAMP) << u[i].x ;
...
//  read the attributes
    ...
    // read the number  of vect
     ...
     {
         value= attribute[key];
         time=_time=(double) atof( value.c_str());
     }
     // read the name 
     key=string("name");
...
return(FVOK);    
     ...
     //FVPoint1D version
size_t FVio::get(FVVect <FVPoint1D<double> >&u, double &time, string &name)
{
    if(_access!=FVREAD)
    {
        cout<<" wrong access code: should be FVREAD"<<endl; return(FVERROR);
...
while(count<_sizevec)// read the data and put it in the valarray
        ...
        if(_nbvec>=2) ignore_val=strtod(ptr, &ptr); // take the second double
        if(_nbvec>=3) ignore_val=strtod(ptr, &ptr); // skip the third double
